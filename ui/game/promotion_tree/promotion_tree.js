...
App.StonehearthPromotionTree = App.View.extend({
    ...
    _buildTreeData: function () {
        ...
        radiant.each(arr, function (i, job) {
            ...
            if (job.description.parent_job) {
                var parent = job.description.parent_job;
                if (!self._jobData[parent]) parent = self._baseWorkerJob; // If we can have this job, but not its parent, reparent to worker.
        ...
        if (inventory_data) {
            ...
            radiant.each(self._jobData, function (jobAlias, jobData) {
                ...
                if (jobData.description) {
                    ...
                    if (talismanUri) {
                        if (inventory_data[talismanUri] && inventory_data[talismanUri].first_item) {
    ...
    _updateUi: function (jobAlias) {
        ...
        var parentJob = job.parent_job && job.parent_job != self._baseWorkerJob ? (self._jobData[job.parent_job] ? self._jobData[job.parent_job].description : null) : null;
        var parentRequiredLevel = job.parent_level_requirement ? job.parent_level_requirement : 0;
    ...
    _calculateRequirementsMet: function (jobAlias) {
        ...
        if (jobDescription.parent_job != undefined) {
            ...
            if (parentJobController != undefined && parentJobController != self._baseWorkerJob) {
    ...
    _updateJobPerks: function (jobAlias) {
        ...
        radiant.each(self._jobData, function (alias, jobData) {
            if (alias != 'stonehearth:jobs:worker' && alias == jobAlias) {
